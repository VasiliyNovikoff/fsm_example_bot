"""
Пример использования состояний в разные моменты взаимодействия с ботом.

Для наглядности в одном файле.

Подробный разбор:
https://stepik.org/lesson/759409/step/3?unit=761425

Типовые ситауции. Дополнительная Теория:

1. Хэндлер должен срабатывать в любых состояния
Это поведение "по умолчанию" для всех хэндлеров. То есть если вы не укажете среди фильтров для хэндлера
никаких фильтров состояний - хэндлер будет доступен в любом состоянии, включая состояние "по умолчанию".

2. Хэндлер должен срабатывать в любых состояниях, кроме состояния "по умолчанию"
То есть мы хотим, чтобы хэндлер работал внутри машины состояний, но не работал вне ее.
Такое поведение достигается через инверсию дефолтного состояния,
то есть мы говорим хэндлеру работать НЕ в дефолтном состоянии - ~StateFilter(default_state)
...
Нужно указать эти конкретные состояния - StateFilter(<конкретное_состояние_1>,
<конкретное_состояние_2>, <конкретное_состояние_3>)
...
Примечание. Исходя из этого, нужно помнить, что если вы используете в вашем боте машину состояний и хотите,
чтобы некоторые команды были доступны только в состоянии по умолчанию (например, команда /start)
- нужно явно указывать дополнительный фильтр StateFilter(default_state),
иначе команда будет доступна вообще в любом состоянии.
"""
